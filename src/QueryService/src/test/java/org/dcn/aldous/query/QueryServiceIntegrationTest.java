...
public class QueryServiceIntegrationTest {
  ...
  @Test
  public void testAppMainScenario() {
    for (int i = 0; i < 1000; i++) {
      new Thread(this::userScenario).start();
    }
  }

  @SneakyThrows(IOException.class)
  protected void userScenario() {
    TestUser user = new TestUser(UUID.randomUUID().toString(), "pass");
    user.register();
    user.signIn();
    for (int i = 0; i < 4; i++) {
      user.addListWithContents();
    }
  }
  ...
  private class TestUser {
    ...
    public void register() {
      ...
      userId = add.readEntity(Integer.class);
    }
    ...
    public void addListWithContents() throws IOException {
